...
public:
   ...
   void             ~CBookRecord(void);
   bool              Create(const uint x1,const string _name,const color _color,const int _X,
...
//| Create                                                           |
//+------------------------------------------------------------------+
bool CBookRecord::Create(const uint x1,const string _name,const color _color,const int _X,
                         const int _Y,const int _X_size,const int _Y_size)
                         //Create(IntegerToString(idx+1),rec_color,curr_x+15,
                          // curr_y,curr_x*3,10))
                        ...
                        if(this.m_price.FontSize(8))
                           if(this.m_price.Color(_color))
                              if(this.m_price.Anchor(ANCHOR_RIGHT_UPPER))
                                 //--- create volume label
                                 if(this.m_vol.Create(0,"vol_"+_name,0,x1+_X+_X_size,_Y))
...
public:
   ...
   //---
   bool              Init(string name,const uint x1,const uint y1,const uint _width,const uint _height);
...
//| Initialization                                                   |
//+------------------------------------------------------------------+
bool CBookBarsPanel::Init(string name,const uint x1,const uint y1,const uint _width,const uint _height)
  {
//--- panel size
   this.m_width=_width;
   ...
   if(this.m_obj_arr.Reserve(this.m_arr_size))
     {
      ...
      if(CheckPointer(ptr_rect_label)==POINTER_DYNAMIC)
         if(ptr_rect_label.Create(0,name,0,x1,y1,this.m_width,this.m_height))
            if(ptr_rect_label.BorderType(BORDER_RAISED))
               if(this.m_obj_arr.Add(ptr_rect_label))
                 {
                  //--- bar label 
                  uint curr_x=x1+this.m_width/5;
                  uint curr_y=35;
                  ...
                  for(uint idx=0;idx<this.m_arr_size;idx++)
                    {
                     ...
                     if(CheckPointer(ptr_record)==POINTER_DYNAMIC)
                        if(ptr_record.Create(x1,name+IntegerToString(idx+1),rec_color,curr_x+15,
                           curr_y,curr_x*2.5,10))
