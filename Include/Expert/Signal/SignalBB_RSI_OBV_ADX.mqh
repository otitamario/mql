...
//| Title=Signals of oscilator 'Bollinger Bands ,RSI,OBV and ADX'             |
//| Type=SignalAdvanced                                              |
//| Name=Bollinger Bands Percent and ADX                                    |
//| ShortName=BBPerc_ADX                                                    |
//| Class=CSignalBBPerc_ADX                                                 |
//|  |
...
//| Parameter=Pattern_0,int,10,peso padrao 0   |
...
//+------------------------------------------------------------------+
class CSignalBBPerc_ADX : public CExpertSignal
  {
protected:
   ...
   CiBands             m_BB_RSI;            // object-oscillator
   //--- adjusted parameters
   
   int               m_period_BB_RSI;      // the "period of calculation" parameter of the oscillator
   int               m_shift_RSI;
   double            m_deviation_RSI;

   CiBands             m_BB_OBV;            // object-oscillator
   //--- adjusted parameters
   
   int               m_period_BB_OBV;      // the "period of calculation" parameter of the oscillator
   int               m_shift_OBV;
   double            m_deviation_OBV;
...
double CSignalBBPerc_ADX::BBP(int ind)
{
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CSignalBBPerc_ADX::CSignalBBPerc_ADX(void) : m_period_BB(18),
                               m_shift(0),
                               m_deviation(1.5),
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CSignalBBPerc_ADX::~CSignalBBPerc_ADX(void)
  {
  }
//+------------------------------------------------------------------+
//| Validation settings protected data.                              |
//+------------------------------------------------------------------+
bool CSignalBBPerc_ADX::ValidationSettings(void)
  {
...
//| Create indicators.                                               |
//+------------------------------------------------------------------+
bool CSignalBBPerc_ADX::InitIndicators(CIndicators *indicators)
  {
//--- check pointer
   if(indicators==NULL)
      return(false);
...
//+------------------------------------------------------------------+
bool CSignalBBPerc_ADX::InitStoch(CIndicators *indicators)
  {
//--- check pointer
   if(indicators==NULL)
      return(false);
...
//--- initialize object
   if(!m_BB.Create(m_symbol.Name(),m_period,m_period_BB,m_shift,m_deviation,m_applied))
...
//+------------------------------------------------------------------+
int CSignalBBPerc_ADX::State(int ind)
  {
   int    res=0;
   double var;
//---
...
//+------------------------------------------------------------------+
bool CSignalBBPerc_ADX::ExtState(int ind)
  {
...
//+------------------------------------------------------------------+
bool CSignalBBPerc_ADX::CompareMaps(int map,int count,bool minimax,int start)
  {
...
//+------------------------------------------------------------------+
int CSignalBBPerc_ADX::LongCondition(void)
  {
   int result=0;
   int idx   =StartIndex();
//---
...
//    if(Diff(idx)>0.0&& ADX(idx)>ADX_minimum)
    if(Diff(idx)>0.0&& ADX(idx)>ADX_minimum && DX_PL_MIN(idx)>0.0)//&&(Diff_PLUS(idx)>0.0||Diff_MINUS(idx)<0.0))
...
//+------------------------------------------------------------------+
int CSignalBBPerc_ADX::ShortCondition(void)
  {
   int result=0;
   int idx   =StartIndex();
//---
   if(Diff(idx)<0.0&& ADX(idx)>ADX_minimum &&DX_PL_MIN(idx)<0.0)//&&(Diff_MINUS(idx)>0.0||Diff_PLUS(idx)<0.0))
     {
