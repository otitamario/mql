...
//--- constants
#define OBJECT_PREFIX       "AD_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   ADHandle=iAD(NULL,_Period,InpVolumeType); 
   if(ADHandle==INVALID_HANDLE)
     {
      Print("The iAD handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,ADBuffer);
...
//--- indicator properties
   string indicatorName="AD_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("AD_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (AD buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(ADHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of ADHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get AD buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(ADHandle,0,0,toCopy,ADBuffer)<=0)
     {
      Print("Getting AD Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(ADBuffer[shift]<=ADBuffer[shift-1] && 
         ADBuffer[shift]<ADBuffer[shift-2] && 
         ADBuffer[shift]<ADBuffer[shift+1])
         //--- if current AD main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(ADBuffer[currentExtremum]>ADBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=ADBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],ADBuffer[currentExtremum],ADBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(ADBuffer[shift]>=ADBuffer[shift-1] && 
         ADBuffer[shift]>ADBuffer[shift-2] && 
         ADBuffer[shift]>ADBuffer[shift+1])
         //--- if current AD main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=ADBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],ADBuffer[currentExtremum],ADBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(ADBuffer[i] <= ADBuffer[i-1] && ADBuffer[i] < ADBuffer[i-2] &&
               ADBuffer[i] <= ADBuffer[i+1] && ADBuffer[i] < ADBuffer[i+2])
               return(i);
