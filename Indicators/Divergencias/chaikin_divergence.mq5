...
//--- constants
#define OBJECT_PREFIX       "CHAIKIN_DivergenceLine"
#define ARROWS_DISPLACEMENT 0.0001
//--- buffers
double bullishDivergence[];
double bearishDivergence[];
double chaikinBuffer[];
//--- handles
int    chaikinHandle=INVALID_HANDLE;
//--- globals variables
...
int OnInit()
  {
//--- indicator handle 
   chaikinHandle=iChaikin(NULL,_Period,chaikin_rapida,chaikin_lenta,InpMAMethod,InpVolumeType); 
   if(chaikinHandle==INVALID_HANDLE)
     {
      Print("The iCHAIKIN handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,chaikinBuffer);
...
//--- indicator properties
   string indicatorName="CHAIKIN_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("CHAIKIN_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (CHAIKIN buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(chaikinHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of chaikinHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get CHAIKIN buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(chaikinHandle,0,0,toCopy,chaikinBuffer)<=0)
     {
      Print("Getting CHAIKIN Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(chaikinBuffer[shift]<=chaikinBuffer[shift-1] && 
         chaikinBuffer[shift]<chaikinBuffer[shift-2] && 
         chaikinBuffer[shift]<chaikinBuffer[shift+1])
         //--- if current CHAIKIN main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(chaikinBuffer[currentExtremum]>chaikinBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=chaikinBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],chaikinBuffer[currentExtremum],chaikinBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(chaikinBuffer[shift]>=chaikinBuffer[shift-1] && 
         chaikinBuffer[shift]>chaikinBuffer[shift-2] && 
         chaikinBuffer[shift]>chaikinBuffer[shift+1])
         //--- if current CHAIKIN main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=chaikinBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],chaikinBuffer[currentExtremum],chaikinBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(chaikinBuffer[i] <= chaikinBuffer[i-1] && chaikinBuffer[i] < chaikinBuffer[i-2] &&
               chaikinBuffer[i] <= chaikinBuffer[i+1] && chaikinBuffer[i] < chaikinBuffer[i+2])
               return(i);
