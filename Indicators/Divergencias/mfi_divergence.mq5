...
//--- constants
#define OBJECT_PREFIX       "MFI_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   mfiHandle=iMFI(NULL,_Period,mfi_period,InpVolumeType); 
   if(mfiHandle==INVALID_HANDLE)
     {
      Print("The iMFI handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,mfiBuffer);
...
//--- indicator properties
   string indicatorName="MFI_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("MFI_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (MFI buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(mfiHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of MFIHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get MFI buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(mfiHandle,0,0,toCopy,mfiBuffer)<=0)
     {
      Print("Getting MFI Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(mfiBuffer[shift]<=mfiBuffer[shift-1] && 
         mfiBuffer[shift]<mfiBuffer[shift-2] && 
         mfiBuffer[shift]<mfiBuffer[shift+1])
         //--- if current MFI main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(mfiBuffer[currentExtremum]>mfiBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=mfiBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],mfiBuffer[currentExtremum],mfiBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(mfiBuffer[shift]>=mfiBuffer[shift-1] && 
         mfiBuffer[shift]>mfiBuffer[shift-2] && 
         mfiBuffer[shift]>mfiBuffer[shift+1])
         //--- if current MFI main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=mfiBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],mfiBuffer[currentExtremum],mfiBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(mfiBuffer[i] <= mfiBuffer[i-1] && mfiBuffer[i] < mfiBuffer[i-2] &&
               mfiBuffer[i] <= mfiBuffer[i+1] && mfiBuffer[i] < mfiBuffer[i+2])
               return(i);
