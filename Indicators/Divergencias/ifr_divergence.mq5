...
//--- constants
#define OBJECT_PREFIX       "IFR_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   ifrHandle=iRSI(NULL,_Period,ifr_period,PRICE_CLOSE); 
   if(ifrHandle==INVALID_HANDLE)
     {
      Print("The iIFR handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,ifrBuffer);
...
//--- indicator properties
   string indicatorName="IFR_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("IFR_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (IFR buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(ifrHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of ifrHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get OBV buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(ifrHandle,0,0,toCopy,ifrBuffer)<=0)
     {
      Print("Getting IFR Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(ifrBuffer[shift]<=ifrBuffer[shift-1] && 
         ifrBuffer[shift]<ifrBuffer[shift-2] && 
         ifrBuffer[shift]<ifrBuffer[shift+1])
         //--- if current IFR main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(ifrBuffer[currentExtremum]>ifrBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=ifrBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],ifrBuffer[currentExtremum],ifrBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(ifrBuffer[shift]>=ifrBuffer[shift-1] && 
         ifrBuffer[shift]>ifrBuffer[shift-2] && 
         ifrBuffer[shift]>ifrBuffer[shift+1])
         //--- if current IFR main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=ifrBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],ifrBuffer[currentExtremum],ifrBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(ifrBuffer[i] <= ifrBuffer[i-1] && ifrBuffer[i] < ifrBuffer[i-2] &&
               ifrBuffer[i] <= ifrBuffer[i+1] && ifrBuffer[i] < ifrBuffer[i+2])
               return(i);
