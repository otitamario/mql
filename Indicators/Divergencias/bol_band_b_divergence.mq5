...
//--- input parameters
//---- input parameters
input int    BBPeriod=15;        //Period
input int    BBShift=0;         // Shift
input double StdDeviation=1.5;  //Standard Deviation
...
//--- constants
#define OBJECT_PREFIX       "BB_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   BBHandle=iCustom(NULL,_Period,"Bollinger_bands_percent",BBPeriod,BBShift,StdDeviation,appliedprc);
 
   if(BBHandle==INVALID_HANDLE)
     {
      Print("The iBB handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
...
//--- indicator properties
   string indicatorName="BB_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("BB_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (BB buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(BBHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of BBHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get OBV buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(BBHandle,0,0,toCopy,BBBuffer)<=0)
     {
      Print("Getting BB Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(BBBuffer[shift]<=BBBuffer[shift-1] && 
         BBBuffer[shift]<BBBuffer[shift-2] && 
         BBBuffer[shift]<BBBuffer[shift+1])
         //--- if current BB main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(BBBuffer[currentExtremum]>BBBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=BBBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],BBBuffer[currentExtremum],BBBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(BBBuffer[shift]>=BBBuffer[shift-1] && 
         BBBuffer[shift]>BBBuffer[shift-2] && 
         BBBuffer[shift]>BBBuffer[shift+1])
         //--- if current BB main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=BBBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],BBBuffer[currentExtremum],BBBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(BBBuffer[i] <= BBBuffer[i-1] && BBBuffer[i] < BBBuffer[i-2] &&
               BBBuffer[i] <= BBBuffer[i+1] && BBBuffer[i] < BBBuffer[i+2])
               return(i);
