...
//--- constants
#define OBJECT_PREFIX       "OBV_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   obvHandle=iOBV(NULL,_Period,InpVolumeType); 
   if(obvHandle==INVALID_HANDLE)
     {
      Print("The iOBV handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,obvBuffer);
...
//--- indicator properties
   string indicatorName="OBV_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("OBV_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (OBV buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(obvHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of OBVHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get OBV buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(obvHandle,0,0,toCopy,obvBuffer)<=0)
     {
      Print("Getting OBV Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(obvBuffer[shift]<=obvBuffer[shift-1] && 
         obvBuffer[shift]<obvBuffer[shift-2] && 
         obvBuffer[shift]<obvBuffer[shift+1])
         //--- if current OBV main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(obvBuffer[currentExtremum]>obvBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=obvBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],obvBuffer[currentExtremum],obvBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(obvBuffer[shift]>=obvBuffer[shift-1] && 
         obvBuffer[shift]>obvBuffer[shift-2] && 
         obvBuffer[shift]>obvBuffer[shift+1])
         //--- if current OBV main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=obvBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],obvBuffer[currentExtremum],obvBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(obvBuffer[i] <= obvBuffer[i-1] && obvBuffer[i] < obvBuffer[i-2] &&
               obvBuffer[i] <= obvBuffer[i+1] && obvBuffer[i] < obvBuffer[i+2])
               return(i);
