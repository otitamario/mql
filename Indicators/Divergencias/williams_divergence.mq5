...
//--- constants
#define OBJECT_PREFIX       "Williams_DivergenceLine"
...
//--- handles
int    williamsHandle=INVALID_HANDLE;
//--- globals variables
...
int OnInit()
  {
//--- indicator handle 
   williamsHandle=iWPR(NULL,_Period,williams_period); 
   if(williamsHandle==INVALID_HANDLE)
     {
      Print("The iWilliams handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
//--- indicator buffers mapping
   SetIndexBuffer(0,bullishDivergence);
   SetIndexBuffer(1,bearishDivergence);
   SetIndexBuffer(2,williamsBuffer);
...
//--- indicator properties
   string indicatorName="Williams_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("Williams_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (Williams buffers) count to copy     
   int toCopy=rates_total-prev_calculated+(prev_calculated<=0 ? 0 : 1);
//--- not all data may be calculated
   int calculated=BarsCalculated(williamsHandle);
   if(calculated<rates_total)
     {
      Print("Not all data of williamsHandle is calculated (",calculated,"bars ). Error",GetLastError());
      return(0);
     }
//--- get OBV buffer
   if(IsStopped()) return(0); //Checking for stop flag
  
   if(CopyBuffer(williamsHandle,0,0,toCopy,williamsBuffer)<=0)
     {
      Print("Getting Williams Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(williamsBuffer[shift]<=williamsBuffer[shift-1] && 
         williamsBuffer[shift]<williamsBuffer[shift-2] && 
         williamsBuffer[shift]<williamsBuffer[shift+1])
         //--- if current Williams main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(williamsBuffer[currentExtremum]>williamsBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence)
           {
            bullishDivergence[currentExtremum]=williamsBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],williamsBuffer[currentExtremum],williamsBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(williamsBuffer[shift]>=williamsBuffer[shift-1] && 
         williamsBuffer[shift]>williamsBuffer[shift-2] && 
         williamsBuffer[shift]>williamsBuffer[shift+1])
         //--- if current Williams main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence)
           {
            bearishDivergence[currentExtremum]=williamsBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],williamsBuffer[currentExtremum],williamsBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(williamsBuffer[i] <= williamsBuffer[i-1] && williamsBuffer[i] < williamsBuffer[i-2] &&
               williamsBuffer[i] <= williamsBuffer[i+1] && williamsBuffer[i] < williamsBuffer[i+2])
               return(i);
