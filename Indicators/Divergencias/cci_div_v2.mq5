...
input int oversold=-100;//Nivel sobrevendido
input int overbought=100;//Nivel sobrecomprado
input bool   drawIndicatorTrendLines = true;
input bool   drawPriceTrendLines     = true;
input bool   displayAlert            = true;
//--- constants
#define OBJECT_PREFIX       "CCI_DivergenceLine"
...
int OnInit()
  {
//--- indicator handle 
   cciHandle=iCCI(NULL,_Period,cci_period,PRICE_CLOSE); 
   if(cciHandle==INVALID_HANDLE)
     {
      Print("The iCCI handle is not created: Error ",GetLastError());
      return(INIT_FAILED);
     }
...
//--- indicator properties
   string indicatorName="CCI_Divergence";
...
void OnDeinit(const int reason)
  {
   ObjectDeleteByName("CCI_DivergenceLine");
  }
//+------------------------------------------------------------------+
...
//--- data (CCI buffers) count to copy     
...
//--- get OBV buffer
   ...
   if(CopyBuffer(cciHandle,0,0,toCopy,cciBuffer)<=0)
     {
      Print("Getting CCI Main is failed! Error : ",GetLastError());
      return(0);
     }
   ...
   for(int shift=start; shift<rates_total-2; shift++)
     {
      ...
      if(cciBuffer[shift]<=cciBuffer[shift-1] && 
         cciBuffer[shift]<cciBuffer[shift-2] && 
         cciBuffer[shift]<cciBuffer[shift+1])
         //--- if current CCI main is a bottom (lower than 2 previous and 1 next)
        {
         currentExtremum=shift;
         lastExtremum=GetIndicatorLastTrough(shift);
         //--- 
         if(cciBuffer[currentExtremum]>cciBuffer[lastExtremum] && 
         ...
         if(isBullishDivergence && cciBuffer[currentExtremum]<=oversold)
           {
            bullishDivergence[currentExtremum]=cciBuffer[currentExtremum]-ARROWS_DISPLACEMENT;
            //---
            ...
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],cciBuffer[currentExtremum],cciBuffer[lastExtremum],Green,divergenceStyle);
            //---
      ...
      if(cciBuffer[shift]>=cciBuffer[shift-1] && 
         cciBuffer[shift]>cciBuffer[shift-2] && 
         cciBuffer[shift]>cciBuffer[shift+1])
         //--- if current CCI main is a top (higher than 2 previous and 1 next)
        {
         currentExtremum=shift;
         ...
         if(isBearishDivergence&&cciBuffer[currentExtremum]>=overbought)
           {
            bearishDivergence[currentExtremum]=cciBuffer[currentExtremum]+ARROWS_DISPLACEMENT;
            //---
            if(drawPriceTrendLines==true)
            ...
            //---
            if(drawIndicatorTrendLines==true)
               DrawTrendLine(TRENDLINE_INDICATOR,time[currentExtremum],time[lastExtremum],cciBuffer[currentExtremum],cciBuffer[lastExtremum],Red,STYLE_SOLID);
            //---
            if(displayAlert==true && shift>=rates_total-3 && time[currentExtremum]!=lastAlertTime)
               DisplayAlert(divergenceMsg,time[currentExtremum]);
           }
   ...
   for(int i=shift-5; i>=2; i--)
     {
            ...
            if(cciBuffer[i] <= cciBuffer[i-1] && cciBuffer[i] < cciBuffer[i-2] &&
               cciBuffer[i] <= cciBuffer[i+1] && cciBuffer[i] < cciBuffer[i+2])
               return(i);
