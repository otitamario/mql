...
//--- calculate ExtFastSC & ExtSlowSC
   ...
   medianRenkoIndicator.SetUseAppliedPriceFlag(PRICE_OPEN);
...
//--- check for rates count
   if(rates_total<ExtPeriodAMA+_begin)
      return(0);
//--- draw begin may be corrected
   if(_begin!=0) PlotIndexSetInteger(0,PLOT_DRAW_BEGIN,ExtPeriodAMA+_begin);
//--- detect position
   int pos=_prev_calculated-1;
//--- first calculations
   if(pos<ExtPeriodAMA+_begin)
     {
      pos=ExtPeriodAMA+_begin;
      for(i=0;i<pos-1;i++) ExtAMABuffer[i]=0.0;
      ExtAMABuffer[pos-1]=medianRenkoIndicator.Price[pos-1];
     }
//--- main cycle
   for(i=pos;i<rates_total && !IsStopped();i++)
     {
      //--- calculate SSC
      double dCurrentSSC=(CalculateER(i,medianRenkoIndicator.Price)*(ExtFastSC-ExtSlowSC))+ExtSlowSC;
      //--- calculate AMA
      double dPrevAMA=ExtAMABuffer[i-1];
      ExtAMABuffer[i]=pow(dCurrentSSC,2)*(medianRenkoIndicator.Price[i]-dPrevAMA)+dPrevAMA;
     }
