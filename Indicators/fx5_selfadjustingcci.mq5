...
//+-----------------------------------+
input uint Length=12; // Период CCI   
...
//---- получение хендла индикатора iCCI
   CCI_Handle=iCCI(NULL,PERIOD_CURRENT,Length,CCIPrice);
   if(CCI_Handle==INVALID_HANDLE) Print(" Не удалось получить хендл индикатора iCCI");
//---- получение хендла индикатора iStdDev
   if(!MA_Method)
     {
      STD_Handle=iStdDev(NULL,PERIOD_CURRENT,Length,0,MODE_SMA,CCI_Handle);
...
//---- инициализация переменной для короткого имени индикатора;
   string shortname;
   StringConcatenate(shortname,"FX5_SelfAdjustingCCI(",Length,")");
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(CCI_Handle,0,0,to_copy,ExtLineBuffer)<=0) return(RESET);
   ...
   if(MA_Method==true)
     {
      for(bar=limit; bar>=0 && !IsStopped(); bar--)
        {
         double smoothedCCI=GetSmoothedCCI(bar);
         ExtCalcBuffer[bar]=MathAbs(ExtLineBuffer[bar]-smoothedCCI);
...
//|                                                                  |
//+------------------------------------------------------------------+
double GetSmoothedCCI(int shift)
  {
//----
   double sum=0;
