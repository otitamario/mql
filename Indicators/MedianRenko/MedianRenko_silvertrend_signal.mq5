   ...
   for(int bar=limit; bar>=0; bar--)
     {
      ...
      Range=0;
      AvgRange=0;
      for(int iii=bar; iii<=bar+SSP; iii++) AvgRange=AvgRange+MathAbs(medianRenkoIndicator.High[iii]-medianRenkoIndicator.Low[iii]);
      Range=AvgRange/(SSP+1);
      //----
      SsMax=medianRenkoIndicator.Low[bar];
      SsMin=medianRenkoIndicator.Close[bar];

      for(int kkk=bar; kkk<=bar+SSP-1; kkk++)
        {
         price=medianRenkoIndicator.High[kkk];
         if(SsMax<price) SsMax=price;
         price=medianRenkoIndicator.Low[kkk];
      ...
      if(medianRenkoIndicator.Close[bar]<smin) uptrend=false;
      if(medianRenkoIndicator.Close[bar]>smax) uptrend=true;

      if(uptrend!=old && uptrend==true)
        {
         BuyBuffer[bar]=medianRenkoIndicator.Low[bar]-Range*0.5;
      ...
      if(uptrend!=old && uptrend==false)
        {
         SellBuffer[bar]=medianRenkoIndicator.High[bar]+Range*0.5;
