...
int OnInit()
  {
   ...
   hma_handle=iCustom(Symbol(),_Period,"MedianRenko\\MedianRenko_ColorHMA",HMA_Period,HMA_Shift);

   lrma_handle=iCustom(Symbol(),_Period,"MedianRenko\\MedianRenko_LRMA_Color",LRMAPeriod);
   ...
   for(int i=MathMax(1,_prev_calculated-1); i<rates_total;i++)
     {
      ...
      if(hma_buffer[i]==2 && LRMA_Color[i]==2 && (hma_buffer[i-1]==1 || LRMA_Color[i-1]==1))
      ...
      if(hma_buffer[i]==1 && LRMA_Color[i]==1 && (hma_buffer[i-1]==2 || LRMA_Color[i-1]==2))
