...
double preco_rsi_up,preco_rsi_down;
static datetime tlastbar;
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
int OnInit()
 ...
 ChartIndicatorAdd(curChartID,1,rsi_handle);

  ulong newChartID=ChartOpen(Symbol(),timeRSI2);
 ChartIndicatorAdd(newChartID,1,rsi_2_handle);
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   CopyTime(NULL,timeRSI2,0,1,TimeNewBar);
   datetime tnewbar=TimeNewBar[0];

   bool isnewbar=tlastbar>0&&tnewbar!=tlastbar;
   ...
   for(int i=MathMax(2,prev_calculated-1); i<rates_total;i++)
     {
      ...
      if(IFR_2Buffer[i-1]<65  &&  IFR_2Buffer[i]>65 && isnewbar)
      {
      preco_rsi_up=close[i];
      Print("PRECO RSI UP ",preco_rsi_up);
      }
      if(IFR_2Buffer[i-1]>32 && IFR_2Buffer[i]<32 && isnewbar)
      {
      preco_rsi_down=close[i];
      Print("PRECO RSI DOWN ",preco_rsi_down);
      }


      if(IFRBuffer[i-2]>65 && IFRBuffer[i]<=65 && IFR_2Buffer[i]>65)
        {
      ...
      if(IFRBuffer[i-2]<32 && IFRBuffer[i]>=32 && IFR_2Buffer[i]<32)
        {
