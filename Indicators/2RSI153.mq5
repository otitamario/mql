   ...
   for(int i=MathMax(2,prev_calculated-1); i<rates_total;i++)
     {
      ...
      if(IFR_2Buffer[i-1]<65  &&  IFR_2Buffer[i]>65 && isnewbar)preco_rsi_up=close[i];
      if(IFR_2Buffer[i-1]>32 && IFR_2Buffer[i]<32 && isnewbar)preco_rsi_down=close[i];


      if(IFRBuffer[i-2]>65 && IFRBuffer[i]<=65 && IFR_2Buffer[i]>50)
        {
      ...
      if(IFRBuffer[i-2]<32 && IFRBuffer[i]>=32 && IFR_2Buffer[i]<25)
        {
